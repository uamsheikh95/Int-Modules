    ...
    def _compute_account_balance(self, accounts, report):
        ...
        if accounts:
            if self.account_report_id != \
                ...
                self._cr.execute('SELECT min(date) FROM account_move_line')
                query_results = self.env.cr.dictfetchone()
                min_date = query_results.get('min') or False
                if min_date:
                    min_date = fields.Date.to_string(min_date)
                self._cr.execute('SELECT max(date) FROM account_move_line')
                query_results = self.env.cr.dictfetchone()
                max_date = query_results.get('max') or False
                if max_date:
                ...
                if accounts[0].user_type_id.include_initial_balance:
                    if self.date_from or self.date_to:
                        if self.date_from and not self.date_to:
                            context.update({'date_to': max_date})
                        if self.date_to and not self.date_from:
                            context.update({'date_from': min_date})
                    context.update({'strict_range': False})
                # For equity
                if report == self.env.ref('account_dynamic_reports.ins_account_financial_report_unallocated_earnings0',
                                          raise_if_not_found=False):
                    if self.date_from:
                        context.update({'strict_range': True, 'initial_bal': True, 'date_to': False})
                    else:
                        context.update({'strict_range': True, 'date_from': min_date, 'date_to': min_date})
