...
class ProductMovesByLocation(models.TransientModel):
    ...
    company_id = fields.Many2one('res.company', string='Company', default=lambda self: self.env['res.company']._company_default_get('mgs_inv.pr_moves_summary'))
    ...
    def confirm(self):
        ...
        return self.env.ref('mgs_inv.action_report_summary_moves').report_action(self, data=data)
...
class ProductMovesByLocationReport(models.AbstractModel):
    ...
    def _sum_qty_by_usage(self,loc_usage, loc_dest_usage, product_id, date_from, date_to):
        params = [loc_usage, loc_dest_usage, product_id, date_from, date_to]
        query = """
            select sum(case
            when sl.usage=%s and sld.usage= %s then qty_done end) as Balance
            from stock_move_line  as sm  left join stock_location as sl on sm.location_id=sl.id
            left join stock_location as sld on sm.location_dest_id=sld.id
            where product_id = %s and sm.date between %s and %s
        """
