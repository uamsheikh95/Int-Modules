...
class ProductMovesHistory(models.TransientModel):
    ...
    company_id = fields.Many2one('res.company', string='Company', default=lambda self: self.env['res.company']._company_default_get('mgs_inv.pr_moves_by_location'))
    ...
    def confirm(self):
        ...
        return self.env.ref('mgs_inv.action_report_location_moves').report_action(self, data=data)
...
class ProductMovesHistoryReport(models.AbstractModel):
    ...
    def _lines(self, product_id, date_from, date_to, stock_location_id1, stock_location_id2, company_id): #, company_branch_id
        full_move = []
        params = [product_id, date_from, date_to, stock_location_id1, stock_location_id2, company_id] #, company_branch_id
        ...
        query = """
            ...
            AND sml.date between %s and %s and (sml.location_id = %s or sml.location_dest_id = %s) and sml.company_id = %s
    ...
    def _sum_open_balance(self, product_id, date_from, stock_location_id1, stock_location_id2, company_id): #, company_branch_id
        params = [product_id, date_from, stock_location_id1, stock_location_id2, company_id] # , company_branch_id
        query = """
            select sum(case
            ...
            left join stock_location as sld on sml.location_dest_id=sld.id
            where sml.product_id = %s and sml.state<>'cancel' and   not (sl.usage='internal' and  sld.usage='internal' )
            and sml.date<%s and (sml.location_id = %s or sml.location_dest_id = %s) and sml.company_id = %s
        """
        #  and company_branch_id = %s
        self.env.cr.execute(query, tuple(params))
