...
class SalesByItemSummaryReport(models.AbstractModel):
    ...
    @api.model
    def balance(self,product, date_from, date_to, company_id, company_branch_id): #, company_branch_id
        full_move = []
        params = [product, date_from, date_to, company_id, company_branch_id] #, company_branch_id
    ...
    @api.model
    def qty(self,product, date_from, date_to, company_id, company_branch_id): #, company_branch_id
        full_move = []
        params = [product, date_from, date_to, company_id, company_branch_id] #, company_branch_id

        query = """
            select cast(sum(sr.product_uom_qty) as INTEGER) as balance
            from  sale_report as sr
            where sr.product_id = %s
            and sr.date between %s and %s and company_id=%s
            and sr.state NOT IN ('draft', 'cancel', 'sent') and sr.invoice_status <> 'no' and sr.company_branch_id=%s
    ...
    def _get_report_values(self, docids, data=None):
        ...
        if product_id:
            ...
            for r in self.env['sale.report'].search([('date', '>=', date_from), ('date', '<=', date_to), ('company_branch_id', '=', company_branch_id)], order="product_id asc"):
                if r.product_id not in product_list and r.price_total is not None:
                    product_list.append(r.product_id)
