...
class SalesByCustomerDetail(models.TransientModel):
    _name = 'mgs_sales_branch.sales_by_cust_detail'
    ...
    company_id = fields.Many2one('res.company', string='Company', default=lambda self: self.env['res.company']._company_default_get('mgs_sales_branch.sales_by_cust_detail'))
    ...
    def confirm(self):
        ...
        return self.env.ref('mgs_sales_branch.action_sales_by_cust_detail').report_action(self, data=data)

class SalesByCustomerDetailReport(models.AbstractModel):
    _name = 'report.mgs_sales_branch.sales_by_cust_detail_report'
