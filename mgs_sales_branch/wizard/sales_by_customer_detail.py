...
class SalesByCustomerDetail(models.TransientModel):
    _name = 'mgs_sales_branch.sales_by_customer_detail'
    ...
    company_id = fields.Many2one('res.company', string='Company', default=lambda self: self.env['res.company']._company_default_get('mgs_sales_branch.sales_by_customer_detail'))
    ...
    def confirm(self):
        ...
        return self.env.ref('mgs_sales_branch.action_sales_by_customer_detail').report_action(self, data=data)

class SalesByCustomerDetailReport(models.AbstractModel):
    _name = 'report.mgs_sales_branch.sales_by_customer_detail_report'
    _description = 'Sales by Customer Detail Report'

    @api.model
    def _lines(self, date_from, date_to, company_id, partner, company_branch_id): #, company_branch_id
        full_move = []
        params = [date_from, date_to, company_id, partner, company_branch_id] #, company_branch_id
        # the old query
        ...
        query = """
            ...
            and sr.partner_id=%s and sr.invoice_status <> 'no' and sr.state NOT IN ('draft', 'cancel', 'sent') and sr.company_branch_id=%s
             order by sr.id
        """
