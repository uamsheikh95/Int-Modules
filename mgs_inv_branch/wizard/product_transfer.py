...
class ProductMovesHistory(models.TransientModel):
	_name = 'mgs_inv_branch.product_transfer'
	...
	company_id = fields.Many2one('res.company', string='Company', default=lambda self: self.env['res.company']._company_default_get('mgs_inv_branch.product_transfer'))
	...
	def confirm(self):
		...
		return self.env.ref('mgs_inv_branch.action_report_product_transfer').report_action(self, data=data)


class ProductMovesHistoryReport(models.AbstractModel):
	_name = 'report.mgs_inv_branch.product_transfer_report'
	_description = 'Product Transfer Report'

	def _lines(self, product_id, date_from, date_to, location_id, location_dest_id, company_branch_id):
		full_move = []
		...
		part_one = """
			...
			AND sm.date between %s and %s and sm.company_branch_id=%s
		"""
	...
	# def render_html(self, docids, data=None):
		...
		# return self.env['report'].render('mgs_inv_branch.product_transfer_report', docargs)
