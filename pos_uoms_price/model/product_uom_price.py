...
class product_uom_price(models.Model):
    ...
    price = fields.Float('Price', required=1, default=0)
    ...
    def create(self, vals):
        ...
        if product_template.uom_id and product_template.uom_id.category_id != unit.category_id:
            raise UserError(
                'Please choice unit the same category of base product unit is %s, for made linked stock inventory' % product_template.uom_id.category_id.name)
